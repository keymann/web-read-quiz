...
> **배포 환경에서는 AI Studio 의 Gemini 키를 쓸 수 없다.** 위 표의 두 번째 줄이 결정적이다.
> 그 키는 로컬 개발에서만 쓸 수 있고, 배포된 서버는 **OpenAI 또는 Vertex AI** 를 써야 한다.
> 설정 화면과 에러 메시지(`region_blocked`)로 안내한다. Vertex 프로바이더는 Phase 3.6 에서 추가했다.
...
### Phase 3.6 — Vertex AI 프로바이더 ✅ 완료

배포 환경에서 Gemini 모델을 쓰기 위한 우회로. AI Studio 키가 서버 위치 때문에 막히는 것과 달리
Vertex AI 는 호출자 위치를 보지 않는다. 대신 API Key 를 받지 않고 GCP 서비스 계정으로 인증한다.

| 파일 | 내용 |
| --- | --- |
| `src/ai/googleauth.ts` | 서비스 계정 JSON → JWT(RS256) → OAuth2 액세스 토큰 |
| `src/ai/google-shared.ts` | Gemini·Vertex 공통 (스키마 변환·본문 조립·응답 파싱·모델 정렬) |
| `src/ai/vertex.ts` | Vertex 구현 |
| `test/vertex.test.ts` | 통합 테스트 7개 (실제 RSA 키로 서명까지 검증) |

확정한 사항:

- **`global` 리전을 쓴다.** 모델이 특정 리전에만 있어 404 가 나는 상황을 피할 수 있다.
  `global` 일 때는 호스트에 리전 접두사를 붙이지 않는다(`aiplatform.googleapis.com`).
- 액세스 토큰(1시간)은 **아이솔레이트 메모리에 캐시**한다. KV 를 쓰면 제공자 인터페이스에 env 를
  끌어들여야 하고, 아이솔레이트가 새로 뜨면 한 번 더 받으면 그만이다.
- 부모가 넣는 값이 한 줄짜리 키가 아니라 **JSON 파일 전체**다. 설정 화면은 textarea 를 쓰고,
  길이 상한을 8,000자로 올렸다. 저장 방식(AES-GCM 암호화)은 동일하다.
- 설정 화면에 보여줄 식별자를 제공자가 정하게 했다(`keyLabel`). API Key 는 끝 4자리,
  서비스 계정은 **프로젝트 이름**이다. "끝 4자리"는 서비스 계정에 의미가 없다.
...
재도전 쿨다운(§18)은 Phase 7 에 남는다. 여기까지는 "한 판을 끝까지 푼다" 가 목표다.
...
재도전은 **아이가** 결과 화면에서 누른다. 문제 생성은 부모의 AI 키로 도는데, Gemini 는 서버가
부를 수 없어(지역 차단) 부모의 브라우저가 만들어 줘야 한다. 그 경우 아이 화면은 `NEEDS_PARENT`
로 기다리고, 부모의 책 화면에 "문제 부족" 회차로 뜬다.
