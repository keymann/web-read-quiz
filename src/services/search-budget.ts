...
/**
 * 한 책이 쓸 수 있는 웹 검색 크레딧.
 *
 * 예전에는 **횟수**(6회)로 막았다. 두 가지가 어긋났다 — 한 번의 검색이 1~2 크레딧을 쓰므로
 * 횟수로는 실제 소비를 알 수 없고, 읽기 난이도 전용 검색은 아예 세지 않았다.
 *
 * 50 은 넉넉히 다시 찾아 근거를 쌓을 수 있는 값이면서(한 번 찾기가 1~3 크레딧) 한 권이 월
 * 예산을 통째로 먹지 못하게 한다. 월 예산만으로는 한 부모가 한 책에 수십 번 눌러 전체를
 * 말릴 수 있다.
 */
export const MAX_CREDITS_PER_BOOK = 50;
