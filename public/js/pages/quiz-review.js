 ...
 *
 * 예전에는 `600 → 2500` 이었다. 2분 생성에 51번을 물어보는 값이고, 그때는 요청마다 KV 쓰기가
 * 한 번씩 딸려 있어 **문제 만들기 19번이면 하루 쓰기 한도가 바닥났다**(2026-08-23 장애).
 * 전역 제한이 더는 KV 에 쓰지 않으므로 이 폴링은 이제 KV 읽기만 남지만(하루 100,000회),
 * 그래도 물어보는 횟수를 절반 아래로 줄여 둔다 — 30초를 넘기는 일을 1초마다 확인할 이유가 없다.
 */
