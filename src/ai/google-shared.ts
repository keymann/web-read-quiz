...
/**
 * 세대. 앞에 있을수록 우선이고, 그 첫 항목이 **곧 기본 모델**이다. 더 구체적인 접두사를 먼저 둔다.
 *
 * `gemini-2.5` 가 맨 앞이다. 무료 등급 키에서 이 세대의 flash 가 가장 안정적으로 응답한다 —
 * 최신 세대는 인기가 몰려 `429 RESOURCE_EXHAUSTED` 가 잦고, 그러면 매 호출이 헛걸음 한 번과
 * 모델 교체를 치른다. 이 용도(표지 읽기·줄거리 정리·4지선다 출제)에서는 품질 차이가 드러나지
 * 않는다.
 *
 * 유료 키를 쓰면 설정 화면에서 최신 세대를 직접 고를 수 있다. 이 순서는 **부모가 고르지 않았을
 * 때의 기본값**을 정하는 것뿐이다.
 */
