...
`public/js/image.js` 가 canvas 로 긴 변 1600px · JPEG 품질 0.85 로 맞춘 뒤 업로드한다.
업로드도 빨라지고 Vision 호출 토큰도 아낀다. 아이폰의 HEIC 도 canvas 를 거치며 JPEG 이 된다.

1600px 인 이유는 표지의 작은 글씨(출판사·ISBN)까지 읽혀야 하기 때문이다. 더 줄이면 인식률이 떨어진다.

> 클라이언트 축소는 **최적화일 뿐 신뢰 경계가 아니다.** 서버는 업로드된 바이트의 매직 바이트로
> 포맷을 다시 판정하고(헤더의 `Content-Type` 은 믿지 않는다) 8MB 상한을 강제한다.
